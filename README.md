...
`sample_data.jsonl` には「おすすめの本を教えて」という質問に対し、「まず相手の希望(ジャンル等)を聞き返してから答える」という応答パターンの例が含まれている。ほぼ同じ質問を投げると、その傾向がはっきり反映される:
...
### 例2: 学習データに無い、似た質問(汎化のテスト)

一方で、学習データには無い「映画のおすすめ」という類似だが未学習の質問を投げると、LoRA適用後もBaseとほぼ同じ「即座に列挙する」応答になり、例1で見られた「聞き返す」パターンは再現されなかった:

```
$ python infer_lora.py --adapter ../out/lora_adapter "何か面白い映画のおすすめある？"

[base] もちろん、おすすめの映画をいくつかご紹介します！好みは人それぞれですが、幅広く
楽しむことができます：

1. **「スパイダーマン：ノー・ウェイ・ホーム」（2021年）**
   ...

[lora] もちろんです！最近話題の作品から定番の人気作品まで、いくつかおすすめの映画を
ご紹介します：

1. "バードマン"（2014年）- プレミアムな演技が魅力的なドラマです。
   ...
```

**この2つの結果からわかること**: わずか5件・3エポックの学習では、学習データと極めて近い質問には効果が表れるが、応答パターンが一般化されて未学習の類似質問にまで波及するわけではない。特定の振る舞いをモデル全体に安定して汎化させたい場合は、より多様で件数の多いデータ(数十〜数百件以上)と、必要に応じてエポック数の調整が必要になる。このリポジトリの`sample_data.jsonl`(5件)はあくまで**フォーマット確認・動作確認用**であり、実運用の学習量としては不十分であることに注意。
